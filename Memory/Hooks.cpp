...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	if (justEnabled) {
		...
		if (enabledTicks > 1 && enabledTicks < 1000) {
			...
			DrawUtils::DrawOutline(vec2_t(282 - (renderCtx->getLineLength(DrawUtils::getFont(Fonts::SMOOTH), &catText, 0.6f) / 2), 3), vec2_t(enabledTicks / 5.5, enabledTicks / 83), MC_Color(184, 0, 255), 1.0);
