...
void Hooks::Init() {
	...
	// Signatures
	{
		...
		void* ConnectionRequest__create = reinterpret_cast<void*>(FindSignature("? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? 48 ? ? ? ? ? ? ? ? ? ? ? E1 ? ? ? ? ? 55"));
...
void* Hooks::Player_tickWorld(C_Player* _this, __int64 unk) {
	...
	if (_this == g_Data.getLocalPlayer()) {
		...
		C_GameMode* gm = *reinterpret_cast<C_GameMode**>(reinterpret_cast<__int64>(_this) + 0x1250);
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	if (justEnabled) {
		...
		if (enabledTicks > 1 && enabledTicks < 1000) {
			...
			DrawUtils::DrawOutline(vec2_t(282 - (renderCtx->getLineLength(DrawUtils::getFont(Fonts::SMOOTH), &catText, 0.6f) / 2), 4.3), vec2_t(enabledTicks / 5.4, enabledTicks / 83), MC_Color(184, 0, 255), 1.0);
