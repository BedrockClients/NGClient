...
bool JsScriptModule::registerCallback(std::wstring callbackName, JsValueRef jsFunc) {
	if (callbackName != L"onWorldTick" && callbackName != L"onEnable" && callbackName != L"onDisable" && callbackName != L"onAttack" && callbackName != L"onRender" && callbackName != L"onRender2d")
		return false;
